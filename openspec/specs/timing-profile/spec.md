# timing-profile Specification

## Purpose

The contract for the `TimingProfile` type, the store's `activeProfile` slot and `setActiveProfile` action, and the `useTimingProfile` hook. Defines the single seam through which the rest of the app reads and swaps the active profile, in preparation for UI-driven profile switching.
