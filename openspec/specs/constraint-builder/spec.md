...
The contract for the `ConstraintBuilder` modal — a live-preview authoring surface for new constraints. Covers the store's modal lifecycle (`builderOpen` / `builderInitial` / `openBuilder` / `closeBuilder`), the five-type form taxonomy, signal-reference editing, real-time solver evaluation in the preview, validation gates, and the submit→addConstraint→close lifecycle. Also specifies the inspector's rewired `+ New constraint` button and the component's placement under `features/`.
