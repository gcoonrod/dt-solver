# signal-builder-decomposition Specification

## Purpose

The contract for the decomposed SignalBuilder sub-directory structure at `src/components/features/signal-builder/`. Specifies the barrel export, sub-module responsibilities (BuilderShell, SBHeader, SBFormType, SBClockParams, SBDataParams, SBTransitionsEditor, SBPreviewWaveform, SBAppearanceRow, constants), and the thin re-export shell at the root path. This is a pure UI decomposition -- no behavioral changes to the SignalBuilder's external API.
