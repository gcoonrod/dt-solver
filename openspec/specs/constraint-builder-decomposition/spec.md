# constraint-builder-decomposition Specification

## Purpose

The contract for the decomposed ConstraintBuilder sub-directory structure at `src/components/features/constraint-builder/`. Specifies the barrel export, sub-module responsibilities (BuilderShell, BuilderHeader, FormType, FormSignalRef, FormBounds, PreviewWaveform, PreviewFooter, constants), and the thin re-export shell at the root path. This is a pure UI decomposition -- no behavioral changes to the ConstraintBuilder's external API.
