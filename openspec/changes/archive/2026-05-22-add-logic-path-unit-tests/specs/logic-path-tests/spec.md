...
### Requirement: Vitest 3.x is the unit-test runner

The repository SHALL adopt Vitest (`^3.0.0`) as the unit-test runner. The dev-dependency set SHALL include `vitest`, `@vitest/coverage-v8`, and `vite-tsconfig-paths`. No other test-runner (Jest, mocha, `node:test`) SHALL be installed in parallel.

#### Scenario: Installation produces a working runner

- **WHEN** a fresh contributor runs `pnpm install`
- **THEN** `pnpm exec vitest --version` SHALL print a 3.x version
